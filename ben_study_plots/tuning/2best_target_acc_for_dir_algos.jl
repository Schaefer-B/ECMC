...
function delta_acc_for_one_algo(delta_tests_arr, nsamples, runs, distribution, dimension, dir_algo, step_var=0.1, var_type = NoVariation(),ch_length=10, jb_refresh=50)
...
function run_all_algos(delta_tests_arr, nsamples, runs, distributions, dimension, dir_algos, step_var=0.1, var_type = NoVariation(), ch_length=10, jb_refresh=50)
    ...
    Threads.@threads for a in 1:algo_count
        idel = Vector{Float64}(undef, dist_count)
        iacc = Vector{Float64}(undef, dist_count)
        for d in 1:dist_count
            idel[d], iacc[d] = delta_acc_for_one_algo(delta_tests_arr, nsamples, runs, distributions[d], dimension, dir_algos[a], step_var, var_type, ch_length, jb_refresh)
        end
        ideal_deltas[a] = idel
        ideal_accs[a] = iacc
    end
...
function plot_accs(ideal_deltas, ideal_accs, direction_algos, distributions, dimension)
    ...
    x_values = [string(dimension,"D ", distributions[i]) for i=eachindex(distributions)]
    ...
    for algo_index in 1:algo_count
        ...
        final_plot = plot!(dist_index .-0.5, ideal_accs[algo_index], lw=2, title=label, subplot=algo_index, xdiscrete_values=x_values)
        #for d in eachindex(distributions)
            
            #annotate!(d, minimum(ideal_accs[algo_index])    , text(string(distributions[d]), :red, :right, 8), subplot=algo_index)
...
distributions = [mvnormal, funnel, mixture]
dimension = 16
nsamples = 5*10^5
runs = 2
...
#---
ideal_deltas, ideal_accs = run_all_algos(delta_tests_arr, nsamples, runs, distributions, dimension, direction_algos)

p = plot_accs(ideal_deltas, ideal_accs, direction_algos, distributions, dimension)
